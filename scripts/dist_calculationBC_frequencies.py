...
#Counting on each sequence for a given number of words and only keeps sequences greater than 27 nucleotides
#Params
...
def calcul_dist_freq(MEFfile, EnteteFile, freqGRFile, distFile):
	...
	for line in open(MEFfile, 'r'):
		...
		TabComptageScore = TabComptageScore.replace([np.inf, -np.inf], 600) #Variable à changer ou a mettre en paramètre pour définir le score maximal	
		...
		#Transformation in frequencies
		freqmot = found_keywords.apply(lambda x: x/sumcomptageCDScible)
		freqall = np.transpose(pd.concat([freqmot.reset_index(drop=True), freqGenomeRef['frequence']], axis=1))
		freqall = freqall.replace([np.inf, -np.inf], 0)
		freqall = freqall.fillna(0)
		...
		if sumcomptageCDScible == 0:
			...
			BC = scipy.spatial.distance.braycurtis(freqall.iloc[0], freqall.iloc[1])
...
calcul_dist_freq(snakemake.input[0], snakemake.input[1], snakemake.input[2], snakemake.output[0])
