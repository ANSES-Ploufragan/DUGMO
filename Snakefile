import os
configfile: "configBC.json"

rule target:
  input:
     "data/HostGenome/CDS_GMO_host.fa",
     config["trainingGMO"] + "_clean.fa",
     expand("data/Rmes/freq_{CDStype}_l{RtailleMot}_m{RMarkovOrder}_{RpourcentSurRep}pSurRep.txt", zip, 
     ...
     expand("{sample}_mixl4m2Propv4l9m7Freq_calculdistBC-CodonUsage.csv", zip, 
        	sample=[config["trainingGMO"] + "_clean", "data/HostGenome/CDS_GMO_host", "data/GMO/CDS_GMO_Inserts"]),
     expand("{inp}_mixl4m2Propv4l9m7Freq_calculdistBC-CodonUsage_notRNA.csv", 
		inp=["data/HostGenome/CDS_GMO_host", config["trainingGMO"] + "_clean"]),
     "data/MachineLearning/Pred-RFPositifs-all.txt"
...
rule trimmomatic_GMO:
  input:
    ...
    R2=config["Reads"]["R2"]
  output:
    R1trim = "data/GMO/GMO_all_R1_001.soft_clean.trimmomatic_trimmed.fastq.gz",
    R1trim_unpaired = temp("data/GMO/GMO_all_R1_001.soft_clean.trimmomatic_trimmed_unpaired.fastq.gz"),
    R2trim = "data/GMO/GMO_all_R2_001.soft_clean.trimmomatic_trimmed.fastq.gz",
    R2trim_unpaired = temp("data/GMO/GMO_all_R2_001.soft_clean.trimmomatic_trimmed_unpaired.fastq.gz")
  conda:
    "envs/Shovill.yaml"
  params:
    logfile="data/GMO/logFile_trimmomatic.log",
    oligofile="data/Oligo/all_illumina_NexteraXT_Truseq_and_NEB.fw_revcomp_v2_20170418_with_phiX174.fa",
...
rule Alignment_Bowtie2_indexGenomeRef_GMO:
  ...
  output:
    BW1bamfile="data/Cleaning_pipeline/Alignment_Bowtie2_GenomeRefGMO/Bowtie2_Alignment_GenomeRefGMO.bam",
    unmappedfq1="data/Cleaning_pipeline/Alignment_Bowtie2_GenomeRefGMO/unalignedreads_GenomeRefGMO.fq.1.gz",
    unmappedfq2="data/Cleaning_pipeline/Alignment_Bowtie2_GenomeRefGMO/unalignedreads_GenomeRefGMO.fq.2.gz"
  conda:
    "envs/Aligner.yaml"
  params:
    index_Gref="data/Cleaning_pipeline/Alignment_Bowtie2_GenomeRefGMO/" + config["Strain"]["RefGenome"],
    threads=config["threads"]
  shell: """
    bowtie2-build --threads {params.threads} -f {input.Gref} {params.index_Gref}
    bowtie2 --un-conc-gz data/Cleaning_pipeline/Alignment_Bowtie2_GenomeRefGMO/unalignedreads_GenomeRefGMO.fq.gz -x {params.index_Gref} -1 {input.R1} -2 {input.R2} -p {params.threads} | samtools sort -O bam -o {output.BW1bamfile}
    """ 
...
rule shovill:
  input:
    ...
    R2=config["Reads"]["R2"]
  output:
    scaffolds="data/Cleaning_pipeline/shovill/contigs.fa",
    scaffoldsP="data/Cleaning_pipeline/shovill_plasmid/contigs.fa"
  conda:
    "envs/Shovill.yaml"
  params:
    folder1="data/Cleaning_pipeline/shovill",
    folder2="data/Cleaning_pipeline/shovill_plasmid"
  shell: """
...
rule Rename_Assemblage: 
  ...
  output:   
    Rename="data/Cleaning_pipeline/shovill/contigs_rename.fasta",
    RenameP="data/Cleaning_pipeline/shovill_plasmid/contigsPlasmid_rename.fasta"
  shell: """
...
rule Annotation_Prokka: 
  ...
  output:
    prokkaFFN="data/Cleaning_pipeline/Annotation_Prokka_AssemblageClean/prokkaAssemblageGMOmincov3.ffn",
    prokkaFFNP="data/Cleaning_pipeline/Annotation_Prokka_AssemblageClean_plasmid/prokkaAssemblagePlasmidmincov3.ffn",
    AnnotFusion="data/Cleaning_pipeline/Annotation_Prokka_AssemblageClean/prokkaAssemblageGMO_Plasmid.fa"
  conda:
    "envs/Prokka.yaml"
  params:
    folder="data/Cleaning_pipeline/Annotation_Prokka_AssemblageClean/",
    folderP="data/Cleaning_pipeline/Annotation_Prokka_AssemblageClean_plasmid/"
  shell: """
...
rule Concat_ProkkaAnnot_plasmid:
  ...
  output:
    AnnotFusionU="data/Cleaning_pipeline/Annotation_Prokka_AssemblageClean/prokkaAssemblageGMO_Plasmid_uniq.fa"
  script:
...
rule Alignment_BWA_indexAnnotationProkka_unalignedGenomeRefGMO:
  input:
    ...
    R1="data/Cleaning_pipeline/Alignment_Bowtie2_GenomeRefGMO/unalignedreads_GenomeRefGMO.fq.1.gz", #rules.Alignment_Bowtie2_indexGenomeRef_GMO.output.unmappedfq1,
    R2="data/Cleaning_pipeline/Alignment_Bowtie2_GenomeRefGMO/unalignedreads_GenomeRefGMO.fq.2.gz" #rules.Alignment_Bowtie2_indexGenomeRef_GMO.output.unmappedfq2
  output:
    BWA2bamfile="data/Cleaning_pipeline/Alignment_BWA_AnnotationGMO_unalignedGenomeRefGMOBWA/BWA_Alignment_AnnotationGMO_unalignedGenomeRefGMOBWA.bam",
    sortedBamBWA2="data/Cleaning_pipeline/Alignment_BWA_AnnotationGMO_unalignedGenomeRefGMOBWA/BWA_Alignment_AnnotationGMO_unalignedGenomeRefGMOBWA_sorted.bam"
  conda:
    "envs/Aligner.yaml"
  params:
    index_Gref="data/Cleaning_pipeline/Alignment_BWA_AnnotationGMO_unalignedGenomeRefGMOBWA/AnnotationGMOBSubtilis_reference",
    threads=config["threads"]
  shell: """
...
rule Consensus_Alignment:
  ...
  output:
    Consensus_fq=temp("data/Cleaning_pipeline/Alignment_BWA_AnnotationGMO_unalignedGenomeRefGMOBWA/final_permissive.fastq"),
    Consensus_fa="data/Cleaning_pipeline/Alignment_BWA_AnnotationGMO_unalignedGenomeRefGMOBWA/final_permissive.fasta",
    ET1=temp("data/Cleaning_pipeline/Alignment_BWA_AnnotationGMO_unalignedGenomeRefGMOBWA/consensus_Alignment_entete1.txt"),
    ET="data/Cleaning_pipeline/Alignment_BWA_AnnotationGMO_unalignedGenomeRefGMOBWA/consensus_Alignment_entete.txt"
  conda:
...
rule CDS_host_and_CDS_GMOInserts_beforeBlastPangenome:
  ...
  output:
    CDS_host="data/Cleaning_pipeline/Alignment_BWA_AnnotationGMO_unalignedGenomeRefGMOBWA/CDS_GMO_host_beforeBlastPangenome.fa",
    CDS_GMOInserts="data/Cleaning_pipeline/Alignment_BWA_AnnotationGMO_unalignedGenomeRefGMOBWA/CDS_GMO_potentialInserts_beforeBlastPangenome.fa"
  conda:
    "envs/Toolsbioinfo.yaml"
  shell: """
...
rule Align_PangenomeCDSdb_GMO:
  ...
  output:
    BlastdbRes="data/Cleaning_pipeline/Blast_Pangenome_CDS/CDS_GMO_potentialinserts_VS_PangenomeCDSdb_percIden95_gapopen3_qcovhsp98.tsv",
    BlastdbR="data/Cleaning_pipeline/Blast_Pangenome_CDS/CDS_GMO_potentialinserts_VS_PangenomeCDSdb_cut.txt"
  conda:
    "envs/Aligner.yaml"
  params:
...
rule Clean_Blast_PangenomeCDS:
  ...
  output:
    CB="data/Cleaning_pipeline/Blast_Pangenome_CDS/CDS_GMO_potentialinserts_VS_PangenomeCDSdb_Clean.txt"
  script:
...
rule PangenomeCDSdb_CDS_host_and_CDS_GMOInserts:
  input:
    ...
    rules.CDS_host_and_CDS_GMOInserts_beforeBlastPangenome.output.CDS_GMOInserts
  output:
    Entete1="data/Cleaning_pipeline/Blast_Pangenome_CDS/CDS_GMO_potentialinserts_VS_PangenomeCDSdb_percIden95_gapopen3_qcovhsp98_EnteteUniq.txt", #temp?
    CDS_GMOpot1="data/Cleaning_pipeline/Blast_Pangenome_CDS/CDS_GMO_potentialinserts_afterBlastPangenomeCDS.fa",
    CDS_host1="data/Cleaning_pipeline/Blast_Pangenome_CDS/CDS_GMO_host_afterBlastPangenomeCDS.fa"
  conda:
    "envs/Toolsbioinfo.yaml"
  shell: """
...
rule Align_PangenomeWholedb_GMO:
  ...
  output:
    BlastdbRes2="data/Cleaning_pipeline/Blast_Pangenome_wholegenome/CDS_GMO_potentialinserts_VS_Pangenomewholedb_percIden95_gapopen3_qcovhsp98.tsv",
    BlastdbR2="data/Cleaning_pipeline/Blast_Pangenome_wholegenome/CDS_GMO_potentialinserts_VS_Pangenomewholedb_cut.txt"
  conda:
    "envs/Aligner.yaml"
  params:
...
rule Clean_Blast_PangenomeWhole:
  ...
  output:
    CB2="data/Cleaning_pipeline/Blast_Pangenome_wholegenome/CDS_GMO_potentialinserts_VS_Pangenomewholedb_Clean.txt"
  script:
    "scripts/Blast_AlignLength.py"
...
rule PangenomeWholedb_CDS_host_and_CDS_GMOInserts:
  input:
    ...
    rules.PangenomeCDSdb_CDS_host_and_CDS_GMOInserts.output.CDS_GMOpot1
  output:
    Entete2="data/Cleaning_pipeline/Blast_Pangenome_wholegenome/CDS_GMO_potentialinserts_VS_Pangenomewholedb_percIden95_gapopen3_qcovhsp98_EnteteUniq.txt",
    CDS_GMOpot2="data/Cleaning_pipeline/Blast_Pangenome_wholegenome/AfterBlastPangenomewholegenome_unalignedSeq_CDS_GMOinserts.fa",
    CDS_host2="data/Cleaning_pipeline/Blast_Pangenome_wholegenome/AfterBlastPangenomewholegenome_alignedSeq_CDS_host.fa"
  conda:
    "envs/Toolsbioinfo.yaml"
  shell: """
    cut -f 1 {input[0]} | uniq > {output[0]}
...
rule MEF_Pangenome_CDS_host_and_CDS_GMOInserts:
    ...
    output:
      CDS_InsertsGMOF="data/GMO/CDS_GMO_Inserts.fa",
      CDS_hostF="data/HostGenome/CDS_GMO_host.fa" 
    shell: """
      cp {input[0]} {output[0]}
...
rule Align_GMOdb_hostGenome:
  input:
    config["trainingGMO"] + ".fa",
    rules.MEF_Pangenome_CDS_host_and_CDS_GMOInserts.output.CDS_hostF   
  output:
    BlastdbRes3="data/TrainingGMO/Blast_CDSGMOhost_VS_GMOdb.tsv"
  conda:
    "envs/Aligner.yaml"
  params:
    name=config["trainingGMO"] + "db",
    evalue=0.00001,
    percidentity=90,
    threads=config["threads"]
  shell: """
...
rule Rmes_calcul_format_allWildCDS:
  input:
    WildCDS="data/HostGenome/{CDStype}" + ".fa"
  output: 
    Rmes1="data/Rmes/{CDStype}.Gauss.m{RMarkovOrder}.mot{RtailleMot}.{RpourcentSurRep}pSurRep.0",
    Rmes2=temp("data/Rmes/{CDStype}.Gauss.m{RMarkovOrder}.mot{RtailleMot}.{RpourcentSurRep}pSurRep.out"),
    Rmes3="data/Rmes/{CDStype}.Gauss.m{RMarkovOrder}.mot{RtailleMot}.{RpourcentSurRep}pSurRep.txt"    
  params:
    name="data/Rmes/{CDStype}.Gauss.m{RMarkovOrder}.mot{RtailleMot}.{RpourcentSurRep}pSurRep", 
...
rule frequence_HostGenome:
  ...
  output:
    freqMot="data/Rmes/freq_{CDStype}_l{RtailleMot}_m{RMarkovOrder}_{RpourcentSurRep}pSurRep.txt"
  conda:
    "envs/R.yaml"
  params:
...
rule calcul_distance_words_Freq:
  input:
    "{sample}-MEFpos3codon.txt",
    "{sample}-Entetepos3codon.txt",
    "data/Rmes/freq_CDS_GMO_host-pos3codon_l{tailleMot}_m{MarkovOrder}_{pourcentSurRep}pSurRep.txt"
  output:
...
rule calcul_distance_words_Propv4:
  input:
    "{sample}-MEF.txt",
    "{sample}-Entete.txt",
    "data/Rmes/freq_CDS_GMO_host_l{tailleMot}_m{MarkovOrder}_{pourcentSurRep}pSurRep.txt"
  output:
...
rule ML_RandomForest:
  input:
    "data/HostGenome/CDS_GMO_host_mixl4m2Propv4l9m7Freq_calculdistBC-CodonUsage_notRNA.csv",
    config["trainingGMO"] + "_clean_mixl4m2Propv4l9m7Freq_calculdistBC-CodonUsage_notRNA.csv",
    "data/GMO/CDS_GMO_Inserts_mixl4m2Propv4l9m7Freq_calculdistBC-CodonUsage.csv"
  conda:
    "envs/R.yaml"
  output:
    "data/MachineLearning/Pred-RFPositifs-all.txt",
    "data/MachineLearning/Pred-RFProba-all.txt"
  script:
